...
def generate_thumbnail(pdf_path: Path, output_path: Path, max_height: int = 400, apng: bool = False) -> bool:
...
def update_bib_file(
    ...
    for entry in entries_sorted:
        ...
        needs_update = (has_pdf and not entry.has_pdf) or (has_preview and not entry.has_preview)
...
Example usage:
  ...
  %(prog)s papers.bib assets/pdf assets/img/publication_preview --dry-run
    ...
    parser.add_argument(
        'preview_dir',
        type=Path,
        help='Directory to store generated preview images'
    )
    ...
    parser.add_argument(
        '--no-update',
        action='store_true',
        help='Do not update the papers.bib file'
    )
